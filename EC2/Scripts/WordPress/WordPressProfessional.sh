#!/bin/bash
# EC2/Scripts/WordPress/WordPressProfessional.sh
...
essential_vars=(
    ...
    "AWS_DB_INSTANCE_DB_NAME_DB"
    "AWS_DB_INSTANCE_SECRET_ARN_DB"
    "WPDOMAIN"
    "ACCOUNT"
    "REGION"
    "AWS_S3_BUCKET_NAME_OFF_LOAD"
    "AWS_S3_BUCKET_NAME_SCRIPT"
)
...
mount_efs() {
    ...
    local efs_ap_id="${AWS_EFS_ACCESS_POINT_ID_0:-}"
    local max_retries=5
...
for var_name in "${essential_vars[@]}"; do
    if [ "$var_name" != "AWS_EFS_ACCESS_POINT_ID_0" ] && [ -z "${!var_name:-}" ]; then
...
mount_efs "$AWS_EFS_FILE_SYSTEM_ID_0" "$MOUNT_POINT"

echo "INFO: Obtendo credenciais do RDS do Secrets Manager..."
# Alterado para usar o ARN completo que o Terraform já entrega e a REGION
